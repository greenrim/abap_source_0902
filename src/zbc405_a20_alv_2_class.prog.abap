...
*           et_index_rows =
      ...
      WHEN 'SCHE'.   "goto flight schedule Report.
        READ TABLE gt_flt INTO gs_flt INDEX ls_row_no-row_id.
        IF sy-subrc = 0.
          SUBMIT bc405_event_d4  "다른 report 를 읽어서 그걸 실행 / 해당 프로그램으로 점프
          AND RETURN
          WITH so_car EQ gs_flt-carrid
          WITH so_con EQ gs_flt-connid. "해당 조건에 맞는 것을 보여주고 돌아와라
        ENDIF.
...
*    CALL METHOD e_object->add_separator. "구분선
*
*    CALL METHOD cl_ctmenu=>load_gui_status "ALV로 데려오기 위한 METHOD
*      EXPORTING
*        program    = sy-cprog
*        status     = 'CT_MENU'    "SCREEN NAME
**       disable    =
*        menu       = e_object     "어디에 넣을건지
*      EXCEPTIONS
*        read_error = 1
*        OTHERS     = 2.
*    IF sy-subrc <> 0.
  ...
  METHOD on_before_user_com.
    CASE e_ucomm.
      WHEN cl_gui_alv_grid=>mc_fc_detail.
        ...
        "mc_fc_detail을 누르면 SCHE 버튼 LOGIC를 탐
