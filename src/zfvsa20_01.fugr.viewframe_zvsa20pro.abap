*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVSA20PRO
*   generation date: 2022.07.21 at 16:20:23
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVSA20PRO           .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVSA20PRO'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVSA20PRO_EXTRACT
TOTAL = ZVSA20PRO_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVSA20PRO'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVSA20PRO-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVSA20PRO_EXTRACT
TOTAL = ZVSA20PRO_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVSA20PRO-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVSA20PRO'
...
TOTAL = ZVSA20PRO_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVSA20PRO-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVSA20PRO-UPD_FLAG NE SPACE.
