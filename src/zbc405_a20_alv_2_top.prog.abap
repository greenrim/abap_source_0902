...
TYPES:   tankcap          TYPE saplane-tankcap,
         ...
         row_color        TYPE c LENGTH 4,  "row_color가 값 갖고있는 record는 색상을 칠함
         light            TYPE c LENGTH 1,
         it_color         TYPE lvc_t_scol, "Table type
         changes_possible TYPE icon-id,
         btn_text         TYPE c LENGTH 10,
         btn_carr         TYPE scarr-carrname,
         it_styl          TYPE lvc_t_styl,
       END OF ts_flt.

DATA : gs_flt      TYPE ts_flt,
       gt_flt      LIKE TABLE OF gs_flt,
       gt_flt_copy LIKE TABLE OF gt_flt.
...
"해당 날짜 기준으로 이전은 변경 불가, 이후는 변경 가능 아이콘
PARAMETERS : p_date TYPE sy-datum DEFAULT '20211001'.
