  ...
  CASE pv_field.
    WHEN 'PRICE'.
      gs_fcat = VALUE #( BASE gs_fcat
                          cfieldname = 'CURRENCY'
                          do_sum = 'X'   "해당 필드에 대해 total sum을 자동 계산해줌
                          ).
    WHEN 'PAYMENTSUM'.
      gs_fcat = VALUE #( BASE gs_fcat
                     cfieldname = 'CURRENCY'
...
FORM set_fcat_layout_pop .
  ...
  gs_layout_pop-no_toolbar = 'X'. "ALV TOOLBER 제거
...
FORM set_carrname .
  ...
  LOOP AT gt_list INTO gs_list.
    ...
    CLEAR : lv_code, lv_msg.  "return 값 받는 변수는 항상 먼저 clear
    ...
    IF lv_code EQ 'S'.
      ...
      "원래 loop를 돌아야하지만 값이 1개만 있는걸 알아서 read table
